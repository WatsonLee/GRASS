...
class Options(object):
    ...
    def __init__(self, data_name = 'twitter'):
        ...
        self.embed_dim = 64
        self.embed_file = '../data/'+data_name+'/dw'+str(self.embed_dim)+'-n2v.txt'
        self.cas_embed_file = '../data/'+data_name+'/cas_di_emb_n2v.txt'
        self.cas_edge_file = '../data/' + data_name + '/cas_graph_di_edges.txt'
...
def LoadDiffusionActions(data_name):
    if not os.path.exists(data_name + "diffusion_graph.csv"):
        ...
        for chunks in cascade_list:
            ...
            pair_user = [(i[0], j[0]) for i, j in zip(userlist[::1], userlist[1::1])]
            if len(pair_user) > 0:
                t_cascades += pair_user

        t_cascades_pd = pd.DataFrame(t_cascades)
        t_cascades_pd.columns = ["user1", "user2"]
        t_cascades_pd.to_csv(data_name + "diffusion_graph.csv", index=False)
    else:
        t_cascades_pd = pd.read_csv(data_name + "diffusion_graph.csv")

    return t_cascades_pd.apply(lambda x: (x["user1"], x["user2"]), axis=1).tolist()

def LoadSocialGraph(data_name):
    options = Options(data_name)
    _u2idx = {}
    _idx2u = []
    ...
    social_edges = LoadDiffusionActions(data_name)
    ...
    for edge in social_edges:
        if edge in tmp_graph.keys():
            tmp_graph[edge] += 1
        else:
            tmp_graph[edge] = 1

    for k,v in tmp_graph.items():
        edges_list.append(k)
        edges_weight_list.append(v)

    edges_list_tensor = torch.LongTensor(edges_list).t()

    data = Data(edge_index=edges_list_tensor, edge_weight=edges_weight_list)

    return data

def LoadHeteGraph(data_name):
    options = Options(data_name)
    _u2idx = {}
    _idx2u = []
    ...
    if os.path.exists(options.net_data):
        with open(options.net_data, 'r') as handle:
            ...
            relation_list_reverse = [edge[::-1] for edge in relation_list]
            topo_edges_list += relation_list_reverse
    else:
        print("No topo data, ERROR!")
        return []


    social_edge_list = LoadDiffusionActions(data_name)

    tmp_graph = dict()

    for edge in social_edge_list:
        if edge in tmp_graph.keys():
            tmp_graph[edge] += 1
        else:
            tmp_graph[edge] = 1
    ...
    for k,v in tmp_graph.items():
        ...
        edge_weight_list.append(v)


    edges_tensor = torch.LongTensor(edge_list).t()
    edges_weight_tensor = torch.FloatTensor(edge_weight_list)
    edges_type_tensor = torch.FloatTensor(edge_type_list)

    data = Data(edge_index=edges_tensor, edges_type=edges_type_tensor, edge_weight=edges_weight_tensor)
...
class DataLoader(object):
    ...
    def __init__(
        ...
        #if self.data == 0:
        #    self._cas_embeds = self._load_ne(self.options.cas_embed_file, self.options.embed_dim)
    ...
    def _readFromFile(self, filename):
        ...
        for line in open(filename):
            ...
            if len(userlist) > 1 and len(userlist)<=Constants.MAX_LEN:
